...
type Action = 'create-job' | 'advance-job' | 'add-part' | 'edit-note' | 'add-customer' | 'add-vehicle' | 'add-stock' | 'adjust-stock' | 'add-invoice' | 'record-payment' | 'send-reminder' | 'add-reminder' | 'settings'

const ROLE_VIEWS: Record<Role, View[]> = {
  Owner: ['dashboard', 'customers', 'vehicles', 'job-cards', 'inventory', 'invoices', 'reports', 'reminders', 'settings'],
  Manager: ['dashboard', 'customers', 'vehicles', 'job-cards', 'inventory', 'invoices', 'reports', 'reminders'],
  Mechanic: ['dashboard', 'job-cards'],
  Receptionist: ['dashboard', 'customers', 'vehicles', 'job-cards', 'invoices', 'reminders'],
  Accountant: ['dashboard', 'customers', 'job-cards', 'invoices', 'reports', 'reminders'],
}

const ROLE_ACTIONS: Record<Role, Action[]> = {
  Owner: ['create-job', 'advance-job', 'add-part', 'edit-note', 'add-customer', 'add-vehicle', 'add-stock', 'adjust-stock', 'add-invoice', 'record-payment', 'send-reminder', 'add-reminder', 'settings'],
  Manager: ['create-job', 'advance-job', 'add-part', 'edit-note', 'add-customer', 'add-vehicle', 'add-stock', 'adjust-stock', 'add-invoice', 'record-payment', 'send-reminder', 'add-reminder'],
  Mechanic: ['advance-job', 'add-part', 'edit-note'],
  Receptionist: ['create-job', 'add-customer', 'add-vehicle', 'send-reminder', 'add-reminder', 'record-payment'],
  Accountant: ['add-invoice', 'record-payment', 'send-reminder', 'add-reminder'],
}

const can = (role: Role, action: Action) => ROLE_ACTIONS[role].includes(action)
const addActionForView: Partial<Record<View, Action>> = { customers: 'add-customer', vehicles: 'add-vehicle', inventory: 'add-stock', invoices: 'add-invoice', reminders: 'add-reminder' }

const PERSONA: Record<Role, { name: string; initials: string; email: string; title: string }> = {
...
function GlobalSearch({ demo, onClose, openJob, setView, views }: {
  demo: Demo; onClose: () => void; openJob: (id: string) => void; setView: (v: View) => void; views: View[]
}) {
  ...
  const results = useMemo(() => {
    ...
    return {
      jobs: views.includes('job-cards') ? demo.state.jobs.filter(j => `${j.id} ${j.customer} ${j.vehicle} ${j.reg}`.toLowerCase().includes(query)).slice(0, 5) : [],
      customers: views.includes('customers') ? demo.state.customers.filter(c => `${c.name} ${c.phone}`.toLowerCase().includes(query)).slice(0, 4) : [],
      vehicles: views.includes('vehicles') ? demo.state.vehicles.filter(v => `${v.reg} ${v.makeModel} ${v.owner}`.toLowerCase().includes(query)).slice(0, 4) : [],
      invoices: views.includes('invoices') ? demo.state.invoices.filter(i => `${i.id} ${i.customer} ${i.vehicle}`.toLowerCase().includes(query)).slice(0, 4) : [],
    }
  }, [query, demo.state, views])
...
function Login({ onLogin, onBack }: { onLogin: (role: Role) => void; onBack: () => void }) {
  ...
  return <main className="login-page"><div className="login-glow" /><div className="login-card"><button onClick={onBack} className="back-link"><ArrowRight className="size-4 rotate-180" /> Back to website</button><Logo light /><div className="login-heading"><div className="eyebrow"><span className="live-dot" /> Demo workspace</div><h1>Welcome back.</h1><p>Choose a workspace view for this presentation.</p></div><label>Email address<input key={role} defaultValue={PERSONA[role].email} type="email" /></label><label>Password<input defaultValue="••••••••••••" type="password" /></label><label>Role<select value={role} onChange={e => setRole(e.target.value as Role)}><option>Owner</option><option>Manager</option><option>Mechanic</option><option>Receptionist</option><option>Accountant</option></select></label><button className="login-button" onClick={() => onLogin(role)}>Enter as {PERSONA[role].name.split(' ')[0]} ({role}) <ArrowRight className="size-4" /></button><div className="demo-garage"><MapPin className="size-4 text-emerald-500" /><div><b>SmartGarage 360 Demo Garage</b><span>Warehouse Road, Industrial Area, Nairobi</span></div></div></div></main>
}
...
function GenericView({ view, demo, openModal, notify, role }: {
  ...
  const entities: Record<'customers' | 'vehicles' | 'inventory' | 'invoices' | 'reminders', React.ReactNode[]> = {
    ...
    inventory: d.stock.filter(s => `${s.name} ${s.partNo} ${s.supplier} ${s.location}`.toLowerCase().includes(q)).map(s => { const low = s.qty > 0 && s.qty <= s.min; const out = s.qty === 0; return <div className="wide-row" key={s.partNo} style={{ gridTemplateColumns: config.grid, minWidth: 760 }}><span className="strong-cell">{s.name}</span><span>{s.partNo}</span><span>{s.supplier}</span><span>{ksh(s.buy)}</span><span>{ksh(s.sell)}</span><span className={cn('font-bold', out ? 'text-red-500' : low ? 'text-amber-600' : 'text-emerald-600')}><span className="inline-flex items-center gap-1.5">{can(role, 'adjust-stock') && <button aria-label="Reduce stock" className="grid size-6 place-items-center rounded border border-slate-200 text-slate-500 hover:bg-slate-100" onClick={() => demo.adjustStock(s.partNo, -1)}><Minus className="size-3" /></button>}{s.qty}{s.unit}{can(role, 'adjust-stock') && <button aria-label="Increase stock" className="grid size-6 place-items-center rounded border border-slate-200 text-slate-500 hover:bg-slate-100" onClick={() => demo.adjustStock(s.partNo, 1)}><Plus className="size-3" /></button>}</span></span><span>{s.location}</span><ChevronRight className="size-4 text-slate-300" /></div> }),
...
export default function Page() {
  ...
  useEffect(() => {
    const handler = (event: Event) => {
      const next = (event as CustomEvent<string>).detail as Role
      setRole(next)
